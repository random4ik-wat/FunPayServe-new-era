...
4. Автоответ на сообщения (+ fuzzy search для опечаток).
5. Автоответ на первые сообщения (+ кастомные приветствия по товарам).
6. Вечный онлайн.
...
11. 🤖 **AI Chat** — умные ответы покупателям через OpenRouter API.
12. 🔧 **AI Diagnostics** — анализ ошибок бота с рекомендациями.
13. 🚨 **Перехват споров** — мгновенный алерт в Telegram.
14. 💰 **Отслеживание баланса** — уведомление при изменении.
15. 📊 **Экспорт продаж** в CSV файл.
16. 📋 **Просмотр логов** через Telegram.
17. 🚫 **Чёрный список** пользователей.
18. 👥 **Несколько админов** Telegram.
19. 🛑 **Graceful Shutdown** — корректное завершение.
20. 🌐 **Health Endpoint** — HTTP мониторинг (`/health`).
21. 🐳 **Docker** — запуск через `docker-compose up -d`.
22. 🔄 **Hot-reload** настроек без перезапуска.
23. 📦 Ротация логов, rate limiter, exponential backoff.
24. 🛡️ **Антифлуд** — игнор спамеров (>5 сообщений за 30с).
25. 🔒 **Анти-дубль заказов** — защита от двойной выдачи при перебоях.
26. 🙏 **Авто-благодарность** — сообщение после успешной выдачи.
27. 🧪 **Тест автоответа** — команда `/test` в Telegram для проверки fuzzy search.
28. 💾 **Бэкап конфигов** — создание архива настроек в один клик.
29. 🔐 **Env Fallback** — чтение ключей из окружения (удобно для Docker).
30. 🎲 **Spintax** — рандомизация автоответов `{Привет|Здравствуй}`.
31. 🚨 **Детект бана** — мгновенный алерт если аккаунт заблокирован.
32. 📈 **График баланса** — ASCII-график изменения баланса в Telegram.
33. ⚡ **`/set` команда** — изменение настроек прямо из Telegram.
34. ⚠️ **Рейтинг покупателей** — предупреждение о подозрительных новичках.
35. 🔌 **REST API** — 4 эндпоинта с авторизацией по ключу (см. `API.md`).
36. 🕵️ **Маскировка токенов** — golden_key, TG token, API keys скрыты в логах.
37. 🎲 **Случайная выдача** — товары рандомным порядком вместо последовательного.
38. 🧪 **Mock режим** — запуск без реальных запросов к FunPay (для отладки).
39. 🏷️ **Динамический watermark** — `{uptime}`, `{sales}`, `{balance}` в авторесп.
40. 📤 **Загрузка товаров .txt** — кинул файл в ТГ → привязал к лоту.
41. 🧙 **CLI Wizard** — пошаговая настройка при первом запуске.
42. 🗄️ **Кеш запросов** — TTL 30с для GET, снижает нагрузку на FunPay.
43. 🔔 **Webhook уведомления** — POST событий на внешний URL (Discord, Slack).
44. 👑 **VIP-команды** — персональные автоответы для конкретных пользователей.
45. 📋 **JSON-логи** — параллельные `.jsonl` логи для машинного анализа.
46. 📑 **Шаблоны автоответов** — готовые пресеты (gaming, digital, services).
47. 🖥️ **Systemd автостарт** — `funpay.service` для Linux серверов.
